...
select is(
  (
    ...
    from (values
      ...
      ('auth_rate_limits'), ('game_answer_submission_receipts'), ('notification_delivery_queue'), ('notification_push_deliveries'), ('push_tokens'),
...
select is((select count(distinct procedure.proname) from pg_proc procedure join pg_namespace namespace on namespace.oid = procedure.pronamespace where namespace.nspname = 'public' and procedure.proname in ('get_admin_portal_context', 'get_admin_profiles_page', 'get_admin_subjects_page', 'get_admin_classrooms_page', 'get_admin_audit_logs_page_secured', 'request_admin_export_job', 'get_admin_export_jobs_page', 'get_admin_account_export_requests_page') and has_function_privilege('authenticated', procedure.oid, 'EXECUTE')), 8::bigint, 'admin supervision and export RPCs are available to authenticated users');
select ok(
  (
    select count(*) = 119
      and count(*) filter (where exists (select 1 from pg_proc procedure join pg_namespace namespace on namespace.oid = procedure.pronamespace where namespace.nspname = 'public' and procedure.proname = expected.name and has_function_privilege('authenticated', procedure.oid, 'EXECUTE'))) = 119
