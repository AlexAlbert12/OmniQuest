...
select ok((select relrowsecurity from pg_class where oid = 'public.avatar_frames'::regclass), 'avatar frame catalog uses RLS');
select ok((select relrowsecurity from pg_class where oid = 'public.profile_cosmetics'::regclass), 'profile cosmetics use RLS');
