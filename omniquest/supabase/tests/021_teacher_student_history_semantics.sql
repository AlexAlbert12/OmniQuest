...
select ok(
  position($needle$manual_review_status not in ('pending', 'in_review', 'needs_changes')$needle$ in pg_get_functiondef('public.get_teacher_student_history_summary(uuid,bigint,bigint,integer)'::regprocedure)) > 0
...
select ok(
  position($needle$manual_review_status in ('pending', 'in_review', 'needs_changes')$needle$ in pg_get_functiondef('public.get_teacher_student_history_summary(uuid,bigint,bigint,integer)'::regprocedure)) > 0
...
select ok(
  position($needle$manual_review_status, 'not_required') not in ('pending', 'in_review', 'needs_changes')$needle$ in pg_get_functiondef('public.get_teacher_student_history_weaknesses(uuid,bigint,bigint,integer)'::regprocedure)) > 0,
