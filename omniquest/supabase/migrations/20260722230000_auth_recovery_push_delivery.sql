-- Align secure teacher-led password recovery with a durable push delivery queue.
-- The queue is populated automatically whenever a persistent notification is created.

create extension if not exists pg_net with schema extensions;
create extension if not exists pg_cron with schema pg_catalog;

-- ---------------------------------------------------------------------------
-- Teacher-requested password recovery audit and rate limiting
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Serialize requests for the same teacher/student pair so concurrent calls
...
begin
  ...
  -- A successfully delivered recovery request is immutable. A later UI,
  -- notification or telemetry failure must never turn it into a failed send.
...
-- ---------------------------------------------------------------------------
-- Durable push delivery queue and per-device delivery records
-- ---------------------------------------------------------------------------
...
-- Only notification content changes enqueue a new delivery. Marking a notification
-- as read does not cause another push.
...
-- ---------------------------------------------------------------------------
-- Optional scheduled invocation. The cron job is safe before secrets exist:
-- it becomes a no-op until Vault contains `project_url` and `push_queue_secret`.
-- ---------------------------------------------------------------------------
...
-- Replace an earlier job with the same name if this migration is replayed locally.
do $$
declare
  v_job_id bigint;
begin
