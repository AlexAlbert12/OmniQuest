...
begin
  ...
  summary as (
    select
      ...
      (select count(*) filter (where total_answers > 0)::int from student_metrics) as evaluated_students,
      ...
      (select coalesce(round(avg(coalesce(ss.max_score, 0))), 0)::int
         from selected_enrollments e
         left join public.subject_scores ss on ss.student_id = e.student_id and ss.subject_id = p_subject_id and ss.classroom_id = v_classroom_id) as average_xp
  )
  select jsonb_build_object(
    'subject', (
      select jsonb_build_object(
        'id', s.id, 'name', s.name, 'description', s.description, 'icon', s.icon, 'code', s.code,
        'educationLevel', s.education_level, 'academicYear', s.academic_year, 'subjectLabel', s.subject_label,
        'themeColor', s.theme_color, 'createdAt', s.created_at
      )
      from public.subjects s where s.id = p_subject_id
    ),
    'classrooms', coalesce((
      select jsonb_agg(jsonb_build_object('id', c.id, 'name', c.name, 'academicYear', c.academic_year, 'active', c.active, 'code', c.code) order by c.created_at asc, c.id asc)
      from public.classrooms c where c.subject_id = p_subject_id and coalesce(c.active, true)
    ), '[]'::jsonb),
    'selectedClassroomId', v_classroom_id,
    'subjectsCount', (
      select count(*)::int from public.subjects s where s.teacher_id = v_teacher_id and not coalesce(s.is_archived, false) and coalesce(s.active, true)
    ),
    'summary', (
      select jsonb_build_object(
        ...
        'activeStudents', evaluated_students,
        'evaluatedStudents', evaluated_students,
        'unassessedStudents', greatest(enrolled_count - evaluated_students, 0),
        ...
        'participation', case when enrolled_count > 0 then round(100.0 * evaluated_students / enrolled_count)::int else 0 end,
    ...
    'recentActivity', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', x.id, 'studentName', x.student_name, 'questionText', x.question_text, 'isCorrect', x.is_correct,
        'earnedPoints', x.earned_points, 'attemptedAt', x.attempted_at
  ...
  select jsonb_build_object(
    'items', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'correctAnswers', correct_answers, 'failedAnswers', failed_answers, 'participation', participation,
    ...
    'gradeDistribution', jsonb_build_array(
      jsonb_build_object('label', 'Excelente (9-10)', 'count', (select count(*)::int from rows where has_activity and grade >= 9)),
      jsonb_build_object('label', 'Notable (7-8,9)', 'count', (select count(*)::int from rows where has_activity and grade >= 7 and grade < 9)),
      jsonb_build_object('label', 'Aprobado (5-6,9)', 'count', (select count(*)::int from rows where has_activity and grade >= 5 and grade < 7)),
      jsonb_build_object('label', 'Suspenso (<5)', 'count', (select count(*)::int from rows where has_activity and grade < 5))
    ),
    'limit', v_limit,
    'offset', v_offset
  ) into v_result;
