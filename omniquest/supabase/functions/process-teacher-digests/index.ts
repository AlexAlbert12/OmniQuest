...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const resend=Deno.env.get('RESEND_API_KEY'); const from=Deno.env.get('MAIL_FROM');
    ...
    for(const digest of (data||[]) as Digest[]){
      try{
        ...
        const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${resend}`,'Content-Type':'application/json'},body:JSON.stringify({from,to:[digest.recipient_email],subject:digest.frequency==='weekly'?'Resumen semanal de OmniQuest':'Resumen diario de OmniQuest',html})})
