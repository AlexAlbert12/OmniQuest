...
Los recorridos autenticados de alumno, profesor y administrador se encuentran en `e2e/web/*-authenticated.spec.ts`. Usan exclusivamente las variables `E2E_STUDENT_EMAIL`, `E2E_STUDENT_PASSWORD`, `E2E_TEACHER_EMAIL`, `E2E_TEACHER_PASSWORD`, `E2E_ADMIN_EMAIL` y `E2E_ADMIN_PASSWORD`. Su `globalSetup` exige las seis variables en la misma terminal, prepara datos idempotentes en Supabase local y cada prueba valida al menos una respuesta real de Supabase antes de ejecutar una acción del rol. La ejecución completa falla de forma inmediata si faltan credenciales, en lugar de omitir silenciosamente los seis recorridos autenticados. La guía completa está en `docs/validation/07-authenticated-web-e2e.md`.
