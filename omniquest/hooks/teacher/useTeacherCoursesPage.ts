...
import { callTeacherRpc, type TeacherCoursesPayload } from '../../lib/teacherServerData'
...
export function useTeacherCoursesPage({ page, pageSize, search, status, sort, enabled = true }: {
  ...
  const load = useCallback(async (refresh = false) => {
    ...
    setError(null)
    try {
      const next = await callTeacherRpc<TeacherCoursesPayload>('get_teacher_courses_page', {
