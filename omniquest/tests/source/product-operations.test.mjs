...
test('usage analytics records game lifecycle, errors and badge unlocks', () => {
  ...
  assert.match(hook, /snapshotPersistenceBlockedRef\.current = true/)
  assert.match(hook, /Promise\.allSettled\(pendingWrites\)/)
  assert.match(hook, /cancelled \|\| snapshotPersistenceBlockedRef\.current/)
