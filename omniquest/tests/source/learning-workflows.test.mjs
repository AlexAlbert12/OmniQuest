...
test('manual review has an owner-only queue, reusable comments, batch decisions and explicit final states', () => {
  const migration = read(migrationPath)
  const simplified = read('supabase/migrations/20260811104500_manual_review_owner_only_simplification.sql')
  ...
  assert.match(simplified, /'not_required', 'pending', 'needs_changes', 'approved', 'rejected'/)
  assert.match(simplified, /create or replace function public\.get_teacher_manual_review_queue/)
  assert.match(simplified, /drop function if exists public\.claim_open_answer_attempt/)
  assert.match(simplified, /create or replace function public\.add_manual_review_comment/)
  assert.match(simplified, /create function public\.review_manual_review_attempt/)
...
test('shared navigation keeps review access and removes obsolete task routes', () => {
  ...
  assert.match(teacherBottomNav, /key: 'reviews'.*href: '\/\(teacher\)\/reviews'/)
  assert.doesNotMatch(teacherBottomNav, /active === 'reviews'.*'students'/)
