...
test('teacher profile is composed from a period-aware aggregate hook and lazy resources', () => {
  ...
  assert.match(metrics, /useAppModal/)
  assert.match(metrics, /teacher\.profile\.participation\.title/)
  assert.match(i18n, /Cómo se calcula la participación/)
  assert.match(metrics, /Precisión global/)
  assert.match(metrics, /responsive\.isMobile/)
  assert.doesNotMatch(metrics, /accessibilityRole="summary"/)
  assert.match(resources, /Consulta los cursos que has creado recientemente\./)
  assert.match(resources, /Consulta las últimas preguntas que has creado\./)
  assert.match(resources, /Mostrar cursos recientes/)
  assert.match(resources, /Mostrar preguntas recientes/)
  assert.match(sidebar, /translatedLabel === item\.labelKey \? item\.label : translatedLabel/)
  for (const key of ['nav.teacher.notifications', 'nav.teacher.settings']) assert.equal(i18n.split(`'${key}'`).length - 1, 2, `${key} must exist in Spanish and English`)
