...
export default function RankingTabs({ value, onChange }: { value: RankingScope; onChange: (scope: RankingScope) => void }) {
  ...
  return (
    <View accessibilityRole="tablist" className="flex-row flex-wrap gap-2 rounded-2xl border border-border-default bg-surface-default p-2">
      {TABS.map((tab) => {
        ...
        return (
          <AppPressable
            ...
            accessibilityHint="Cambia el periodo de la clasificación"
            accessibilityRole="tab"
            accessibilityState={{ selected }}
            onPress={() => onChange(tab.scope)}
            className="min-h-[44px] min-w-[116px] flex-1 flex-row items-center justify-center gap-2 rounded-xl px-3 py-2"
          ...
          >
            ...
            <Text className={selected ? 'font-black text-brand-student' : 'font-bold text-text-secondary'}>{tab.label}</Text>
          </AppPressable>
        )
      })}
    </View>
