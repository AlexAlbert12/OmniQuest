...
export default function StudentPrimaryLearningCTA({
  ...
  const isStackedMobile = stackActionOnMobile && width < 640

  return (
    <Pressable onPress={onPress} disabled={disabled} className={className} accessibilityRole="button" accessibilityLabel={ctaLabel} style={({ pressed }) => ({ opacity: disabled ? 0.58 : pressed ? 0.86 : 1 })}>
      <LinearGradient colors={['#2B176F', '#171B50', '#0A1A35']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={{ borderRadius: 24, borderWidth: 1, borderColor: withAlpha(color, '70'), overflow: 'hidden' }}>
        {isStackedMobile ? (
          <View className="relative p-4">
            <View className="absolute -right-10 -top-10 h-32 w-32 rounded-full" style={{ backgroundColor: withAlpha(color, '22') }} />
            <View className="flex-row items-start gap-3">
              <View className="h-14 w-14 items-center justify-center rounded-2xl" style={{ backgroundColor: withAlpha(color, '24') }}>
                <Ionicons name={icon} size={28} color={color} />
              </View>
              <View className="min-w-0 flex-1">
                <Text className="text-[18px] font-black text-white" numberOfLines={2}>{title}</Text>
            ...
            <View className="mt-3 flex-row flex-wrap items-center justify-between gap-3">
              {meta ? <Text className="min-w-0 flex-1 text-[12px] font-black" style={{ color }}>{meta}</Text> : <View className="flex-1" />}
              <View className="min-h-[44px] flex-row items-center justify-center gap-2 rounded-2xl px-4" style={{ backgroundColor: color }}>
