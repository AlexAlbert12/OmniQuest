...
export default function StudentListRow({
  ...
  return (
    <Container
      onPress={onPress}
      className={`flex-row items-center gap-4 rounded-xl border border-border-subtle bg-surface-raised p-3 ${className}`}
    ...
    >
      ...
      <View className="min-w-0 flex-1">
        ...
        {subtitle ? <Text className="mt-1 text-[12px] text-text-secondary" numberOfLines={1}>{subtitle}</Text> : null}
        {children}
      </View>
      {meta.length > 0 ? (
        <View className="hidden flex-row items-center gap-4 md:flex">
          {meta.map((item) => (
            <View key={item.label} className="border-l border-border-subtle pl-4">
              <Text className="text-[12px] text-text-disabled">{item.label}</Text>
              <Text className="text-[13px] font-bold" style={{ color: item.color || '#DDE7F4' }}>{item.value}</Text>
            </View>
          ))}
        </View>
      ) : null}
      {actionLabel ? <Text className="hidden text-[12px] font-black text-brand-student md:flex">{actionLabel}</Text> : null}
