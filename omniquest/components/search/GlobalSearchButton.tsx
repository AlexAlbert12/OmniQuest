...
export default function GlobalSearchButton({ role, compact = false }: GlobalSearchButtonProps) {
  ...
  return <>
    <Pressable accessibilityLabel="Abrir búsqueda global" accessibilityRole="button" accessibilityHint={Platform.OS === 'web' ? 'También puedes usar Control o Comando más K' : undefined} hitSlop={8} onPress={() => setVisible(true)} style={({ pressed }) => [styles.trigger, compact ? styles.triggerCompact : styles.triggerWide, { borderColor: colors.border, backgroundColor: colors.surface, opacity: pressed ? 0.76 : 1 }]}>
      <Ionicons name="search-outline" size={compact ? 21 : 18} color={triggerIconColor} />
      {!compact ? <>
      <Text style={[styles.triggerLabel, { color: colors.text }]}>Buscar</Text>
      </> : null}
    </Pressable>
...
function StateBox({ accent, icon, text, loading, danger }: { accent: string; icon: IconName; text: string; loading?: boolean; danger?: boolean }) { const { colors, tokens } = useAppTheme(); return <View style={styles.stateBox}>{loading ? <ActivityIndicator color={accent} /> : <Ionicons name={icon} size={30} color={danger ? tokens.semantic.danger : accent} />}<Text style={[styles.stateText, { color: colors.textMuted }]}>{text}</Text></View> }
const SearchResultRow = React.memo(function SearchResultRow({ accent, result, onSelect, query }: { accent: string; result: SearchResult; onSelect: (result: SearchResult) => void; query: string }) {
  const { colors, tokens } = useAppTheme()
    ...
    <Pressable
      ...
      style={({ pressed }) => [
        styles.resultRow,
        {
          ...
          backgroundColor: pressed ? tokens.surface.selected : tokens.background.secondary,
          opacity: pressed ? 0.88 : 1,
        },
      ]}
    >
      <View style={[styles.resultAccent, { backgroundColor: meta.color }]} />
      <View style={[styles.resultIcon, { backgroundColor: `${meta.color}26` }]}><Ionicons name={meta.icon} size={21} color={meta.color} /></View>
      <View style={styles.resultCopy}>
        <HighlightedText value={result.title} query={query} baseStyle={[styles.resultTitle, { color: colors.text }]} highlightColor={accent} numberOfLines={1} />
        <HighlightedText value={result.subtitle || meta.label} query={query} baseStyle={[styles.resultSubtitle, { color: colors.textMuted }]} highlightColor={accent} numberOfLines={2} />
      </View>
      <View style={[styles.resultArrow, { backgroundColor: `${accent}18` }]}><Ionicons name="chevron-forward" size={17} color={accent} /></View>
    </Pressable>
  )
})
