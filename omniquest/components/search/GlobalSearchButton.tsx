...
export default function GlobalSearchButton({ role, compact = false }: GlobalSearchButtonProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      setError(null)
      try {
        const { data, error: searchError } = await supabase.rpc('search_app_entities' as any, { p_query: normalizedQuery, p_limit: PAGE_SIZE, p_offset: offset })
        ...
        if (!cancelled) { if (offset === 0) setResults([]); setError(searchError?.message || 'No se pudo realizar la búsqueda.') }
  ...
  const close = () => { setVisible(false); setQuery(''); setResults([]); setOffset(0); setTotal(0); setError(null) }
  const rememberSearch = async (value: string) => { const updated = [value, ...recentSearches.filter((item) => item.toLowerCase() !== value.toLowerCase())].slice(0, MAX_RECENT_SEARCHES); setRecentSearches(updated); await AsyncStorage.setItem(storageKey, JSON.stringify(updated)) }
  const clearRecent = async () => { setRecentSearches([]); await AsyncStorage.removeItem(storageKey) }
