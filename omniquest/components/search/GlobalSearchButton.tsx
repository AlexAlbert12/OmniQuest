...
const SearchResultRow = React.memo(function SearchResultRow({ accent, result, onSelect, query }: { accent: string; result: SearchResult; onSelect: (result: SearchResult) => void; query: string }) { const { colors } = useAppTheme(); const meta = getResultMeta(result); const handlePress = useCallback(() => onSelect(result), [onSelect, result]); return <Pressable accessibilityLabel={`${result.title}. ${result.subtitle || meta.label}`} accessibilityRole="button" onPress={handlePress} style={({ pressed }) => [styles.resultRow, { borderColor: colors.border, backgroundColor: pressed ? colors.surfaceMuted : colors.backgroundAlt }]}><View style={[styles.resultIcon, { backgroundColor: `${meta.color}22` }]}><Ionicons name={meta.icon} size={21} color={meta.color} /></View><View style={styles.resultCopy}><HighlightedText value={result.title} query={query} baseStyle={[styles.resultTitle, { color: colors.text }]} highlightColor={accent} numberOfLines={1} /><HighlightedText value={result.subtitle || meta.label} query={query} baseStyle={[styles.resultSubtitle, { color: colors.textMuted }]} highlightColor={accent} numberOfLines={2} /></View><Ionicons name="chevron-forward" size={18} color={accent} /></Pressable> })
