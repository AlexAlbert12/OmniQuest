  ...
  return (
    <AdminScaffold activeSection="home" title="Inicio Admin" subtitle="Vista general del sistema, alertas y accesos rápidos." data={data}>
      ...
      <View className="mt-5 flex-row flex-wrap gap-4">
        {can('users.manage') ? <HomeShortcut icon="person-add-outline" label="Crear profesor" onPress={() => actions.router.push('/(admin)/teachers' as any)} /> : null}
        {can('courses.read') ? <HomeShortcut icon="archive-outline" label="Cursos archivados" onPress={() => actions.router.push('/(admin)/courses?archived=1' as any)} /> : null}
        {can('users.export') || can('audit.export') ? <HomeShortcut icon="download-outline" label="Exportaciones" onPress={() => showAlert('Exportaciones asíncronas', 'Los listados grandes se generan como trabajos y aparecen más abajo.')} /> : null}
        {can('courses.read') ? <HomeShortcut icon="albums-outline" label="Revisar clases" onPress={() => actions.router.push('/(admin)/classrooms' as any)} /> : null}
        {can('support.read') ? <HomeShortcut icon="headset-outline" label="Gestionar soporte" onPress={() => actions.router.push('/(admin)/support' as any)} /> : null}
        {can('audit.read') ? <HomeShortcut icon="receipt-outline" label="Ver auditoría" onPress={() => actions.router.push('/(admin)/audit' as any)} /> : null}
      </View>
