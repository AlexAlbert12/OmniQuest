...
function AdminAuditTable({ rows }: { rows: AdminAuditLogRow[] }) {
  ...
  return (
    <View style={[styles.table, { borderColor: tokens.border.default }]}>
      ...
      {rows.map((log, index) => <AdminAuditRow key={`${log.id}-${log.created_at}`} log={log} alternate={index % 2 !== 0} />)}
    </View>
  )
}
...
const AdminAuditRow = React.memo(function AdminAuditRow({ log, alternate }: { log: AdminAuditLogRow; alternate: boolean }) {
  const { tokens } = useAppTheme()
