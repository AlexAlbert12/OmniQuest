...
import { AppBottomSheet, AppButton, AppDropdown, AppPressable, DateCalendar } from '../../ui'
  ...
  return (
    <AdminScaffold activeSection="push" requiredPermissions={['notifications.read']} title="Notificaciones push" subtitle="Supervisa la cola, entregas, reintentos y dispositivos registrados sin exponer tokens internos." data={data}>
      <View className={responsive.isDesktop ? 'flex-row items-start justify-between gap-4' : 'gap-3'}>
        ...
        {canManage ? <AppButton label="Enviar prueba a mi dispositivo" icon="phone-portrait-outline" variant="secondary" loading={center.acting} onPress={center.sendTest} fullWidth={!responsive.isDesktop} /> : null}
      </View>
...
function PushDateField({ boundary, label, onChange, value }: { boundary: 'start' | 'end'; label: string; onChange: (value: string | null) => void; value: string | null }) {
  ...
  return <View style={{ minWidth: 170, flex: 1 }}><Text className="mb-2 text-[12px] font-bold text-text-secondary">{label}</Text><AppPressable accessibilityLabel={`Fecha ${label.toLowerCase()}`} onPress={() => { setMonth(selectedDate || new Date()); setVisible(true) }} style={({ pressed }) => ({ minHeight: 48, borderWidth: 1, borderRadius: 13, paddingHorizontal: 13, flexDirection: 'row', alignItems: 'center', gap: 9, borderColor: tokens.border.default, backgroundColor: tokens.surface.interactive, opacity: pressed ? 0.82 : 1 })}><Ionicons name="calendar-outline" size={18} color={tokens.text.secondary} /><Text className="min-w-0 flex-1 text-[13px] font-extrabold text-text-primary">{formatted}</Text><Ionicons name="chevron-down" size={17} color={tokens.text.muted} /></AppPressable><AppBottomSheet visible={visible} onClose={() => setVisible(false)} title={`Fecha ${label.toLowerCase()}`} footer={<View className="flex-row justify-end gap-2"><AppButton label="Sin límite" icon="close-circle-outline" variant="ghost" onPress={() => { onChange(null); setVisible(false) }} /><AppButton label="Cancelar" variant="secondary" onPress={() => setVisible(false)} /></View>}><DateCalendar month={month} selectedDate={selectedDate} onMonthChange={setMonth} onSelectDate={select} subtitle="Selecciona un día" locale={locale} minimumDate={null} /></AppBottomSheet></View>
}
