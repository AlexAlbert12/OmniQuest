...
export function AdminStudentsSection() {
  ...
  const applyGovernance = async (result: AdminGovernanceResult) => {
    ...
    if (governanceMode === 'delete-student-progress' && progressProfile) await actions.deleteStudentProgress(progressProfile, result.reason)
    selection.clear(); setProgressProfile(null); setGovernanceMode(null); page.refresh()
  }
  ...
  return (
    <AdminScaffold activeSection="students" title="Alumnos" subtitle="Supervisa cuentas, seguridad, inscripciones y actividad académica." data={data}>
      <AdminUsersSection listArea={<Panel title="Listado de alumnos" icon="people-outline" className="mt-5">
        <AdminSearchBar search={search} onChangeSearch={setSearch} placeholder="Buscar alumno por nombre o correo..." exporting={exporting || exportJobs.loading} onExport={canExport && !isMobile ? () => void handleExport() : undefined} />
        ...
        <View className="mt-4">{page.loading && !page.refreshing ? <ListLoadingState /> : null}<VirtualizedStack data={page.rows} keyExtractor={(profile) => profile.id} renderItem={(profile) => <ProfileRowCard profile={profile} meta={`${profile.enrollment_count ?? 0} ${(profile.enrollment_count ?? 0) === 1 ? 'inscripción' : 'inscripciones'}`} selected={selection.isSelected(profile.id)} onToggleSelected={canManage ? () => selection.toggle(profile.id) : undefined} actions={[
          ...
          { label: 'Eliminar progreso', icon: 'trash-outline', destructive: true, disabled: !canManage, onPress: () => { setProgressProfile(profile); setGovernanceMode('delete-student-progress') } },
      ...
      {confirmation.modal}
      <AdminGovernanceModal visible={Boolean(governanceMode)} mode={governanceMode || 'deactivate-user'} count={governanceMode === 'delete-student-progress' ? 1 : selection.count} targetLabel={governanceMode === 'delete-student-progress' ? progressProfile?.alias : undefined} onCancel={() => { setProgressProfile(null); setGovernanceMode(null) }} onConfirm={applyGovernance} />
