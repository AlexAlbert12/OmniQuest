  ...
  return (
    <AdminScaffold activeSection="support" title="Soporte" subtitle="Gestiona solicitudes, conversación, SLA y coordinación interna desde una única cola." data={data}>
      {selectedTicket ? <Panel title={`Gestionar ticket #${selectedTicket.id}`} icon="chatbubble-ellipses-outline" className="mt-5">
        {!isDesktop ? <View className="mb-4"><AppBackButton label="Volver a soporte" role="admin" onPress={closeTicket} /></View> : null}
        <View className={isDesktop ? 'flex-row gap-5' : 'gap-4'}>
          ...
          <View className={isDesktop ? 'w-[420px]' : ''}>
            ...
            <Text className="mt-4 text-[12px] font-black uppercase tracking-[0.7px] text-text-muted">Prioridad</Text><View className="mt-2 flex-row flex-wrap gap-2">{(['low', 'medium', 'high'] as const).map((priority) => <AdminChoiceChip key={priority} active={editPriority === priority} label={t(`support.priority.${priority}`)} tone={SUPPORT_PRIORITY_TONES[priority]} useDefaultBorder onPress={() => setEditPriority(priority)} />)}</View>
            <Text className="mt-4 text-[12px] font-black uppercase tracking-[0.7px] text-text-muted">Etiquetas</Text><View className="mt-2 flex-row flex-wrap gap-2">{directory.tags.map((tag) => <AdminChoiceChip key={tag.slug} active={selectedTagSlugs.includes(tag.slug)} label={tag.label} tone="admin" useDefaultBorder onPress={() => setSelectedTagSlugs((current) => current.includes(tag.slug) ? current.filter((value) => value !== tag.slug) : [...current, tag.slug])} />)}</View>
            ...
            <View className="mt-4 flex-row flex-wrap justify-end gap-2"><AppBackButton label="Volver a la cola" role="admin" onPress={closeTicket} /><AdminButton label={saving ? 'Guardando...' : notifiesUser ? 'Guardar y notificar' : 'Guardar cambios'} icon="save-outline" loading={saving} disabled={saving || !canManage} onPress={() => void saveTicket()} /></View>
          </View>
        </View>
      </Panel> : null}
