  ...
  return (
    <AdminScaffold activeSection="support" title="Soporte" subtitle="Gestiona solicitudes, conversación, SLA y coordinación interna desde una única cola." data={data}>
      ...
      <Panel title="Cola de tickets" icon="headset-outline" className="mt-5">
        ...
        <View className="mt-4" style={{ gap: 12 }}>{supportPage.loading && !supportPage.refreshing ? <ListLoadingState /> : null}{supportPage.rows.map((ticket) => <View key={ticket.id}><SupportTicketCard ticket={ticket} onManage={() => void openTicket(ticket)} /><View className="mt-2 flex-row flex-wrap gap-2 px-2"><SlaPill state={ticket.sla_state} />{ticket.assigned_admin_alias ? <MiniPill icon="person-outline" label={ticket.assigned_admin_alias} /> : null}{(ticket.tags || []).map((tag) => <MiniPill key={tag.slug} icon="pricetag-outline" label={tag.label} />)}</View></View>)}{!supportPage.loading && supportPage.rows.length === 0 ? <EmptyState label="No hay tickets que coincidan con los filtros." /> : null}</View>
