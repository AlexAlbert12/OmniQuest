      ...
      {items.map((item) => (
        <View key={`${item.subject_id}:${item.topic_id ?? 'general'}`} className="rounded-2xl border p-4" style={{ borderColor: tokens.border.default, backgroundColor: tokens.surface.default }}>
          <View className="flex-row flex-wrap items-center gap-3">
            <View className="h-11 w-11 items-center justify-center rounded-xl" style={{ backgroundColor: tokens.semanticSurface.warning }}><Ionicons name="warning-outline" size={22} color={tokens.semantic.warning} /></View>
            <View className="min-w-[220px] flex-1"><Text className="font-black" style={{ color: tokens.text.primary }}>{item.topic_title}</Text><Text className="mt-1 text-[12px]" style={{ color: tokens.text.muted }}>{item.subject_name}</Text></View>
            <View className="items-end"><Text className="text-[20px] font-black" style={{ color: tokens.semantic.warning }}>{item.accuracy_percent}%</Text><Text className="text-[11px]" style={{ color: tokens.text.muted }}>{formatCount(item.mistakes, 'error', 'errores')} · {formatCount(item.attempts, 'intento', 'intentos')}</Text></View>
          </View>
        </View>
      ))}
    </View>
