...
export default function MultipleChoiceEditor({
  ...
  onChangeAnswer: (text: string, index: number) => void
  ...
  return (
    <QuestionFormSection
      ...
      subtitle="Completa las opciones y marca una única respuesta correcta."
      icon="list-outline"
    >
      ...
      <View className="mt-2 flex-row flex-wrap gap-2">
        {[2, 3, 4, 5, 6].map((count) => {
          ...
          return (
            <AppPressable
              key={count}
              accessibilityLabel={`${count} opciones`}
              accessibilityState={{ selected: active }}
              onPress={() => onOptionsCountChange(count)}
              className="h-10 min-w-[46px] items-center justify-center rounded-xl border px-3"
              style={{
                borderColor: active ? tokens.brand.teacher : tokens.border.default,
                backgroundColor: active ? tokens.brand.teacher : tokens.surface.interactive,
              }}
            >
              <Text className="font-black" style={{ color: active ? '#FFFFFF' : tokens.text.secondary }}>{count}</Text>
            </AppPressable>
          )
        })}
      </View>
      <View className="mt-4 gap-3">
        {answers.map((answer, index) => (
          <ChoiceAnswerRow
            ...
            onChangeText={(value) => onChangeAnswer(value, index)}
          />
        ))}
      </View>
    </QuestionFormSection>
